    ...
    while (true) {
      if (_esp->available()) {
        ...
        if (expectedIdx != -1) {
          ...
          int lnvIdx = response.indexOf("link is not valid");
          if (failIdx != -1 || lnvIdx != -1) {
            _debugPrintln("FAIL!!");
            _assureTCPReady();
            break;
          }
...
void VegemiteSandwich::registerHandler(AsyncTimer *timer) {
  ...
  timer->setInterval(
      [this]() {
        ...
        while (_esp->available()) {
          response = response + _esp->readString();
          delay(60);
        }
        if (response.length() > 0) {
          _debugPrintln("##=DEBUG: Received from TCP: " + response + " =##");
          while (true) {
            ...
            _debugPrintln("DEBUG: Received data: " + dataID + " = " +
                          String(dataValue));
...
float VegemiteSandwich::infer(const char dataID[]) {
  return get(dataID, "GET+inference");
}
...
float VegemiteSandwich::get(const char dataID[], const char action[]) {
  ...
  if (String(action).indexOf("+set-zero") > 0) {
    ...
    _setDocElem(dataID, action, VEGEMITE_UNKNOWN);
  }
